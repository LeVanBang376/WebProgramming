    ...
    if(array_key_exists('3', $url)){
        ...
        if($method == 'GET'){
            try{
                ...
                if(count($query) == 2){
                    if(in_array('id', $query)){
                        ...
                        echo ProductController::getProductById($news_id);
            ...
            try{
                ...
                if($data and array_key_exists('title', $data) and array_key_exists('price', $data) and array_key_exists('thumbnail', $data) 
                        and array_key_exists('description', $data) and array_key_exists('discount', $data)){ 
                ...
                if(count($query) == 2){
                    if(in_array('id', $query)){
                        ...
                        if($data and array_key_exists('title', $data) and array_key_exists('price', $data) and  array_key_exists('thumbnail', $data)
                            and array_key_exists('description', $data) and  array_key_exists('discount', $data)){
                            $auth = Auth::checkAuth(['admin']);
                            echo ProductController::updateProduct($product_id, $data);
                            http_response_code(200);
                        }
                    } else {
                        echo json_encode(['message' => 'Invalid input']);
